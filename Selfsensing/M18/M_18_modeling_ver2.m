...
for m=1:num_iter
    for n=1:num_iter
        Tor1_(m,n)=3/2*P_n*(psi_a*i_q(m,n)+(L_d_d(11,11)-L_q_q(11,11))/1000*i_d(m,n)*i_q(m,n));
        Tor2_(m,n)=3/2*P_n*(psi_a*i_q(m,n)+(L_d_d(m,n)-L_q_q(m,n))/1000*i_d(m,n)*i_d(m,n));
        Tor2_(m,n)=3/2*P_n*(flux_d(m,n)*i_q(m,n)-flux_q(m,n)*i_d(m,n));
%         Tor2_(m,n)=3/2*P_n*((L_d_d(m,n)-L_q_q(n,m))/1000*i_d(n,m)*i_d(m,n));
        Tor3_(m,n)=3/2*P_n*(flux_d(m,n)*i_d(m,n)-flux_q(m,n)*i_d(m,n));

        Tor3_(m,n)=3/2*P_n*(flux_d(m,n)*i_q(m,n));
        Tor4_(m,n)=3/2*P_n*(-flux_q(m,n)*i_d(m,n));%+flux_d(n)*i_d(m)-flux_q(m)*i_q(n));
%         Tor4_(m,n)=3/2*P_n*(-flux_q(m,n)*i_d(m,n));%+flux_d(n)*i_d(m)-flux_q(m)*i_q(n));

%         Tor4_(m,n)=3/2*P_n*(flux_d(m)*i_q(n)-flux_d(n)*i_q(n)+flux_q(n)*i_d(n)-flux_q(m)*i_d(m));%+flux_d(n)*i_d(m)-flux_q(m)*i_q(n));
%         Tor4_(m,n)=3/2*P_n*(flux_q(n)*i_d(n));%+flux_d(n)*i_d(m)-flux_q(m)*i_q(n));
%         Tor4_(m,n)=3/2*P_n*(flux_q(m)*i_d(m));%+flux_d(n)*i_d(m)-flux_q(m)*i_q(n));
