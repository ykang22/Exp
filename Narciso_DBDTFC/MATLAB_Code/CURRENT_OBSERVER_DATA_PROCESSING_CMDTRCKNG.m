%% CURRENT OBSERVER DATA PROCESSING COMMAND TRACKING

%% NEGATIVE FREQUENCIES
% load('curr_obs_dynstff_neg_1.mat');
iqe_hat = detrend(data1(2:end,5));
ide_hat = detrend(data1(2:end,6));
...
% iqe_hat = detrend(data1(2:end,11));
% ide_hat = detrend(data1(2:end,12));
% iqe      = detrend(data1(2:end,9));
% ide      = detrend(data1(2:end,10));
% 
% % clear data1
...
% iqd      = iqe - 1i*ide;
% 
% [H,fh] = mscohere(iqd,iqd_hat,rectwin(1000),50,2^16,20000,'centered');
% [T,f] = tfestimate(iqd,iqd_hat,rectwin(1000),50,2^16,20000,'centered');
% 
% % n = length(f);
% % 
% % k = 0;
...
figure(2), semilogx(-fn(1:n/2), abs(Tn(1:n/2)),...
...
%     'LineWidth',1.5), grid on, ...%ylim([0.1 10]),...
%     xlim([min(f(length(f)/2:end)) max(f)]);
